...
AUTHOR : RAGHAV GUPTA
DATE   : 14 july 2019
AIM    : implement merge sort using link list functions
STATUS : !! success !!!
##############################################################################*/
...
node* mergeSort(node* a){
	...
	// rec case
		// 1. split the list
	node* mid = getMidPoint(a);
	node* b = mid->next;
	mid->next = NULL;
		// 2. sort recursively
	a = mergeSort(a);
	b = mergeSort(b);
		// 3. merge them and return the head of sorted list
	return mergeSortedListsREC(a, b);
}


int main(){

// input and print the list
node *head = NULL;
cin>>head;
cout<<head;
